"""
Run Evaluation with FIXED Ground Truth

Uses the improved ground truth annotations that filter out false PERSON entities.
"""
...
def main():
    print("\n" + "="*80)
    print("PHI DETECTION EVALUATION (WITH FIXED GROUND TRUTH)")
    print("="*80)
    
    # File paths - USING FIXED GROUND TRUTH
    pred_file = "outputs/detected_entities.json"
    true_file = "data/annotated/annotations_1000_fixed.json"
    ...
    if not Path(true_file).exists():
        print(f"\n❌ Fixed ground truth file not found: {true_file}")
        print("   Run fixed ground truth generation first:")
        print("   python scripts/generate_ground_truth_fixed.py")
        sys.exit(1)
    ...
    print(f"  Ground truth (FIXED): {len(true_data)} documents")
    ...
    old_recall = 0.6974
    new_recall = relaxed_results['recall']
    
    improvement_vs_baseline = (new_recall - baseline_recall) / baseline_recall * 100
    improvement_vs_old = (new_recall - old_recall) / old_recall * 100
    
    print(f"Baseline Recall (150 notes, 4 scenarios):  {baseline_recall:.2%}")
    print(f"Old Recall (with flawed ground truth):     {old_recall:.2%}")
    print(f"NEW Recall (1245 notes, 12 scenarios):     {new_recall:.2%}")
    print(f"\n📈 Improvement vs Baseline: {improvement_vs_baseline:+.1f}%")
    
    if improvement_vs_old > 0:
        print(f"📈 Improvement vs Old:      {improvement_vs_old:+.1f}%")
    
    # Save results
    output_file = "outputs/evaluation_results_fixed.json"
    results = {
        ...
        "baseline_comparison": {
            ...
            "improvement_vs_baseline_percent": round(improvement_vs_baseline, 2),
            "improvement_vs_old_percent": round(improvement_vs_old, 2)
        }
    }
