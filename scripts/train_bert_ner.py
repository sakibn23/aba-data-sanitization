"""
Fine-tune dslim/bert-base-NER on PERSON entities from the synthetic ABA notes.
...
Output: scripts/detectors/bert_ner_model/   (saved tokenizer + model weights)

Usage:
    python scripts/train_bert_ner.py
    python scripts/train_bert_ner.py --epochs 5 --batch-size 8 --output-dir scripts/detectors/bert_ner_model
"""
...
try:
    ...
    print("   Install with: pip install torch transformers datasets seqeval accelerate")
    sys.exit(1)

# ── constants ───────────────────────────────────────────────────────────────
BASE_MODEL   = "dslim/bert-base-NER"   # pre-trained on CoNLL-2003 NER
...
def _align_labels(offsets, person_spans):
    ...
    for tok_start, tok_end in offsets:
        ...
        in_person = any(
            tok_start >= ps and tok_end <= pe
            for ps, pe in person_spans
        )
        if in_person:
            # B-PERSON at span start or after an O token
            is_begin = (not prev_in_person) or any(
                tok_start == ps for ps, _ in person_spans
            )
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune BERT NER for PERSON detection.")
    ...
    parser.add_argument("--output-dir",      default="scripts/detectors/bert_ner_model")
    ...
    print(f"\n{'='*80}")
    print("BERT NER FINE-TUNING  —  PERSON entity detection")
    print(f"{'='*80}\n")
    ...
    trainer = Trainer(
        ...
        data_collator=data_collator,   # tokenizer is already inside data_collator
    ...
    # save label map so bert_detector can load it without the base model
    ...
    print("\nNext step:")
    print("  python scripts/run_phi_detection.py --detector bert")
    print(f"\n{'='*80}\n")
